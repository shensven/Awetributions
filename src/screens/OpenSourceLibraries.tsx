...
const OpenSourceLibraries: React.FC = () => {
  ...
  const renderTouchableRipple = ({item}: {item: TouchableRippleProps}) => {
    return (
      <TouchableRipple rippleColor={PaperColors.ripple} onPress={() => openLink(item.repository)}>
        <List.Item
          title={item.name}
          titleStyle={[styles.item_title, {color: PaperColors.text}]}
          right={() => (
            <View style={styles.item_right}>
              <Text style={[styles.item_right_type, {color: PaperColors.placeholder}]}>{item.license}</Text>
